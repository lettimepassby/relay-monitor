...
// 尝试把提供商返回的时间标签解析成时间戳；解析失败返回 null，前端按原始标签展示
function parseDateLabel(s) {
  if (!s) return null;
  let t = Date.parse(s);
  if (Number.isNaN(t)) t = Date.parse(String(s).replace(" ", "T"));
...
function normSub2Trend(trend) {
  return (trend || []).map((p) => ({
    t: parseDateLabel(p.date),
    label: String(p.date ?? ""),
...
async function sub2apiUsage(station, { startMs, endMs, granularity, tz }) {
  const base = trimBase(station.baseUrl);
  if (!base) throw new Error("缺少站点地址");
  const params = {
    start_date: localDateStr(startMs),
    end_date: localDateStr(endMs),
    granularity,
    timezone: tz,
  };
  ...
  if (snap.status < 300 && snap.body?.code === 0 && snap.body.data) {
    ...
    return { models: normSub2Models(d.models), trend: normSub2Trend(d.trend) };
  }
  ...
  return {
    models: mo.status < 300 && mo.body?.code === 0 ? normSub2Models(mo.body.data?.models) : [],
    trend: tr.status < 300 && tr.body?.code === 0 ? normSub2Trend(tr.body.data?.trend) : [],
  };
}
