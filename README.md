...
- **总览面板**：总剩余余额、日均总消耗、低余额 / 耗尽、查询异常统计；每站余额、用量进度条、状态标签、查询延迟
...
- **通知告警**：余额偏低 / 耗尽 / 查询失败 / 恢复正常 / 预计 N 天内耗尽，状态迁移触发、自动去重、可配重复提醒间隔；支持 8 种渠道：
  Telegram、钉钉（含加签）、企业微信、飞书（含签名）、Bark、ntfy、Server酱（含 sctp 新版）、自定义 Webhook，每个渠道可单独测试
...
│   ├── notify.js        8 种通知渠道发送器（纯 fetch，无 SDK）
